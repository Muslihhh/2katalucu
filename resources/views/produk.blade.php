        ...
        <div class="lg:w-1/2 sm:w-full p-5 bg-gray-100">
            <style>
                .swiper-container.gallery-top {
                    ...
                    position: relative;
                    overflow: hidden;
                    /* Atur posisi agar navigasi tidak tergeser */
                }

                .swiper-slide {
                    width: 100%;
                    /* Pastikan tiap slide mengambil lebar penuh */
                    height: 100%;
                    display: flex;
                ...
                .swiper-wrapper {
                    display: flex;
                    /* Menyusun slide secara horizontal */
                }
                ...
                .gallery-thumbs .swiper-slide-thumb-active img {
                    border-color: blue;
                    /* Warna border untuk gambar aktif */
                }
            </style>
        ...
        <div class="lg:w-1/2 sm:w-full grid content-between">
            <div class="h-auto">
                ...
                <div class="flex gap-2 items-center mt-4">
                    @if ($product->discount > 0)
                        ...
                        <p class="discounted-price text-2xl text-blue-700">
                ...
                <div class="flex items-center">
                    @if ($averageRating > 0)
                        <span>({{ number_format($averageRating, 1) }} <i class=" fa fa-star text-yellow-400"></i>)
                            {{ $comments->count() }} {{ Str::plural('Review', $comments->count()) }}</span>
                ...
                <a href="http://">
                    <p class="">Kategori : {{ $product->category->name ?? 'N/A' }}</p>
                </a>
                <p>Deskripsi : </p>
            ...
            <div class="flex gap-5 h-auto">
                ...
                <a href="{{ route('order.form', ['product' => $product->id]) }}" class="btn-order w-1/2">
                    <button
                        class=" fa fa-whatsapp w-full  h-14 transition duration-150 ease-in-out hover:scale-110 bg-green-500 hover:bg-green-600 text-white font-bold py-3 rounded-md ">
                        Order</button>
                </a>
            </div>
        </div>
    ...
    <div class="mt-20 border-t pt-10 border-black">
        ...
        @auth
            <form class=" mt-5" action="{{ route('comments.store', $product) }}" method="POST">
                    ...
                    @for ($i = 1; $i <= 5; $i++)
                        ...
                        <label for="star{{ $i }}" class="cursor-pointer">
                            <svg xmlns="http://www.w3.org/2000/svg"
                                fill="{{ $i <= $product->rating ? '#f59e0b' : 'none' }}" viewBox="0 0 24 24"
                                stroke="{{ $i <= $product->rating ? '#f59e0b' : '#d1d5db' }}" class="w-6 h-6 star"
                                data-value="{{ $i }}">
                ...
                <button type="submit"
                    class=" mb-8 bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded-md">Tambahkan
                    Ulasan</button>
            </form>
        @else
            <p>Anda Perlu <a href="{{ route('login') }}" class=" hover:underline">Login</a> atau <a
                    href="{{ route('registrasi') }}" class=" hover:underline">Register</a> untuk menambahkan Ulasan</p>
        @endauth
        <div class="comment-count mt-4">
            {{ $comments->count() }} {{ Str::plural('Review', $comments->count()) }}
        </div>
        @foreach ($comments as $comment)
            ...
            <div
                class="grid bg-gray-50 border-b py-10 px-6 border-black items-center w-full text-sm text-gray-900 dark:text-white">
                <div class="flex mb-2">
                    <img class="mr-4 w-12 h-12 rounded-full" src="{{ asset('user.png') }}"
